...
> [!NOTE]
> 첫 실행 시 관리자 암호를 한 번 입력하게 됩니다. macOS에서는 관리자 권한을 가진 프로그램만 덮개를 닫은 상태에서 잠자기를 막을 수 있습니다 → [설정](#설정)
...
Homebrew 6는 서드파티 탭을 불러오기 전에 명시적인 신뢰를 요구하며, `brew trust`가 그 동의를 기록합니다. 건너뛰면 "Refusing to load cask ... from untrusted tap"으로 설치가 중단됩니다.
...
macOS에서는 관리자 권한을 가진 프로그램만 덮개를 닫은 상태에서 잠자기를 막을 수 있기 때문입니다. 설치되는 헬퍼는 macOS 기본 전원 설정(`pmset`)을 전환하는 일만 합니다 — 네트워크 통신도, 데이터 수집도 없습니다. 설정에서 언제든지 제거할 수 있습니다.
...
| 로그인 시 열기 | Mac 시작과 함께 실행합니다 |
| 덮개를 닫으면 디스플레이 끄기 | 잠자기를 막는 동안 macOS는 덮개를 닫아도 내장 디스플레이를 켜 둡니다. 디스플레이를 끄면 불필요한 전력 소모를 줄일 수 있습니다. 외장 디스플레이가 연결되어 있으면 적용되지 않습니다 |
...
상태 판별은 두 CLI가 문서화하지 않은 내부 상태 파일을 읽는 방식이므로, **해당 도구가 업데이트되면 동작하지 않을 수 있습니다**. 동작하지 않으면 [이슈](https://github.com/yamamoto7/vibe-awake/issues)로 알려 주세요.
